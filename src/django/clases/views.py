...
def create_group(request):
    ...
    return render(
        request, "clases/create_grupo_form.html", {"form": form, "formset": formset}
    )
...
def update_group(request, pk):
    ...
    if request.method == "POST":
        ...
        formset = [
            BloqueDeClaseForm(request.POST, prefix=str(form), instance=bloque)
            for i, bloque in enumerate(bloque_de_clase_instances)
        ]
        if form.is_valid() and all([f.is_valid() for f in formset]):
            ...
            for f in formset:
                ...
                bloque.hora_inicio = datetime.strptime(
                    f.cleaned_data["hora_inicio"], "%H:%M"
                ).time()
                bloque.hora_fin = datetime.strptime(
                    f.cleaned_data["hora_fin"], "%H:%M"
                ).time()
        ...
        formset = [
            BloqueDeClaseForm(request.POST or None, prefix=str(i), instance=bloque)
            for i, bloque in enumerate(bloque_de_clase_instances)
        ]
